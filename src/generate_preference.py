...
def load_real_data_Amask(
    ...
    """Load real data from the specified paths"""
    m_A = np.load(male_data_path + "_A.npy")
    m_mask = np.load(male_data_path + "_mask.npy")
    f_A = np.load(female_data_path + "_A.npy")
    f_mask = np.load(female_data_path + "_mask.npy")

    return m_A, m_mask, f_A, f_mask
